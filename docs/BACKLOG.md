...
| Sector 8/9 confrontation cinematic (multi-panel) | Item 4 (§A8 Phase 2) | **Partially wired (2026-08-12).** `codex/mentor-system-character` delivered 8 composited placeholder panels (4 entrance, 4 final — see `src/assets/cutscenes/README.md`), built from the existing calm/corrupted sprites, explicitly meant to be replaced panel-for-panel once Step 3b is sent. Only `rogue-entrance-04.png` is wired so far, as a new `RogueSprite` `confronting` state swapped into m8-1's existing boss-moment aside (`MissionView.tsx`). The other 7 panels (`rogue-entrance-01..03`, all 4 `rogue-final-*`) are merged but **not yet wired** — no Sector 9 finale moment exists in the code to hang them on yet; that's a real design decision (new UI, not just an asset swap), not something to guess at solo. Step 3b is still worth sending for the real replacements regardless. | `src/assets/cutscenes/rogue-{entrance,final}-0{1-4}.png` |
| `rogue-named.png`, `nameplate-{echo,rogue}.png` | Not yet assigned to an item | **Delivered (2026-08-11) via `codex/mentor-system-character`, not yet wired.** No code references these anywhere — there's no existing "ROGUE gets named" story beat or nameplate UI to attach them to. Needs a decision on where these belong before implementing, not a guess. | `src/assets/rogue/rogue-named.png`, `src/assets/ui/nameplate-{echo,rogue}.png` |
...
| "Good AI" mentor/tutor character sprite (ECHO) | Item 3, Learn SQL Mode (`claude/learn-sql-mode`) | **`calm` state wired (2026-08-12).** `codex/mentor-system-character` delivered `echo-idle.png` (a placeholder, composited rather than a fresh Claude Design export) — wired into `MentorSprite.tsx`'s existing `calm` state slot, which was purpose-built for exactly this swap (see the component's own comment). The `explaining` state still has no second pose and keeps its CSS placeholder. Step 3c (`docs/GAME_DESIGN_BRIEF.md` §B) is still worth sending for real art and a second pose — remember to name the character "ECHO" in that prompt. Requesting/sending Step 3c does not itself approve item 3, which is still blocked on the separate AI-tutor approval decision above. | `src/assets/mentor/echo-idle.png`; `src/components/MentorSprite.tsx` |
