...
| "Good AI" mentor/tutor character sprite | Item 3 | **Prompt drafted 2026-08-10** — not yet sent. `docs/GAME_DESIGN_BRIEF.md` §B Step 3c. Requesting the art does not approve or start item 3 itself, which is still blocked on the separate AI-tutor approval decision above. **Note (2026-08-11): a separate in-flight build (`codex/mentor-system-character`) has since named this character "ECHO"** and shipped a CSS placeholder for it — add that name to the Step 3c prompt before sending so the delivered art and the shipped placeholder agree. | — |
